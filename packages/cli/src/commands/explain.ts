 ...
 */
import { Config, findConfig, loadConfig, resolveActions, RoleConfig, getInheritChain } from './utils.js'
...
export async function explain(args: string[]): Promise<void> {
    ...
    for (const [roleName, roleDef] of Object.entries(resource.roles)) {
        ...
        const roleDef = resource.roles[roleName]!
        const own = roleDef.actions?.join(', ') ?? ''
        const inherited = resolved.filter((a: string) => !(roleDef.actions ?? []).includes(a))
